...
export function validateHeaderStructure(children: AstChild[]): AnalyzerResult[] {
...
function isTitlePresent(markdownFile: string, children: AstChild[], configuration: FrontmatterConfiguration): boolean {
  const header = children.find((child) => child.type === 'Header' && child.depth === 1);
  if (!header) {
...
function hasDuplicateTitle(
  markdownFile: string,
  children: AstChild[],
  frontmatterConfiguration: FrontmatterConfiguration,
): boolean {
  const frontmatter = matter(markdownFile);
  const firstLevelHeadline = children.find((child) => child.type === 'Header' && child.depth === 1);
...
export function validateTitle(
  ...
  if (hasDuplicateTitle(markdownFile, children, frontmatterConfiguration)) {
    analyzerResults.push(
      new AnalyzerError('Article Title', 'Found title in First-Level Headline and Frontmatter', ResultType.body),
    );
  }
