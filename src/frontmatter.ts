...
function firstKeywordMultipleSeoDescriptionOccurrences(
  configuration: FrontmatterConfiguration,
...
function validateSeoDescriptionWithAllKeywords(
  configuration: FrontmatterConfiguration,
  seoDescription: string,
  keywords: string[],
): AnalyzerResult[] {
...
function validateSeoDescription(
  ...
  if (seoDescription && seoDescription.toLowerCase().indexOf(keyword.toLowerCase()) === -1) {
    results.push(
      new AnalyzerError(configuration.seoDescriptionField, `Keyword '${keyword}' not found`, ResultType.frontmatter),
    );
  }
  if (seoDescription && seoDescription.length > 160) {
    results.push(
      new AnalyzerError(
        configuration.seoDescriptionField,
        'SEO Description should 160 characters max.',
        ResultType.frontmatter,
      ),
    );
  }
...
function validateSeoTitleWithAllKeywords(
  configuration: FrontmatterConfiguration,
  seoTitle: string,
  keywords: string[],
): AnalyzerResult[] {
...
function validateSeoTitle(
  ...
  if (seoTitle && seoTitle.toLowerCase().indexOf(keyword.toLowerCase()) === -1) {
    results.push(
      new AnalyzerError(configuration.seoTitleField, `Keyword '${keyword}' not found`, ResultType.frontmatter),
    );
  }
  if (seoTitle && seoTitle.length > 60) {
    results.push(
      new AnalyzerError(
        configuration.seoTitleField,
        'SEO Title should have 60 Characters max.',
        ResultType.frontmatter,
      ),
    );
  }
