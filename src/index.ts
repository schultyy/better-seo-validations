...
import { ResultType } from './errors/resultType';
import { AnalyzerResult } from './errors/analyzerResult';
...
export function runAnalysis(markdownFile: string, configuration: FrontmatterConfiguration): AnalyzerResult[] {
  const keywords = extractKeywords(markdownFile);
  return analyze(markdownFile, keywords, configuration).concat(
    analyzeFrontmatter(markdownFile, configuration, keywords),
  );
}

function validateFirstParagraph(children: AstChild[], keyword: string): AnalyzerResult[] {
  const analyzerResults = [];
  const firstParagraph = children.find((child) => child.type === 'Paragraph');
  const text = firstParagraph?.children?.find((child) => child.type === 'Str');
...
function validateParagraphLength(children: AstChild[]): AnalyzerResult[] {
  const paragraphs = children.filter((child) => child.type === 'Paragraph');
  const longParagraphErrors = paragraphs
    .filter((paragraph) => {
      return paragraph.raw.split(/\s+/).length >= 200;
    })
    .map((paragraph) => {
      return new ParagraphError(
        'Paragraph',
        paragraph.loc,
        `Paragraph starting with ${paragraph.raw.substr(0, 20)} has more than 200 characters(${
...
function validateArticleLength(children: AstChild[]): AnalyzerResult[] {
  const paragraphs = children
    .filter((child) => child.type === 'Paragraph')
    .filter((child) => child.children)
    .flatMap((child) => child.children?.filter((grandChild) => grandChild.type === 'Str'));

  const totalLength = paragraphs
    .flatMap((textElement) => (textElement ? textElement.raw.split(/\s+/) : []))
    .reduce((acc, _currentValue) => acc + 1, 0);
  if (totalLength < 300) {
    return [
      new AnalyzerError(
        'Article Length',
        `Article is too short. Expected: At least 300 Characters. Actual Length: ${totalLength}`,
        ResultType.body,
      ),
    ];
  }
